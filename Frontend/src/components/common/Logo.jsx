...
const Logo = ({ onClick, style, className = '', iconOnly = false, iconClassName = '' }) => {
  return (
    <div className={`logo-container ${className} ${iconOnly ? 'icon-only' : ''}`} onClick={onClick} style={style}>
      <div className="logo-icon-wrapper">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className={`logo-icon ${iconClassName}`}
          fill="none"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      {!iconOnly && <span className="logo-text">BloomPortfolio</span>}
    </div>
  );
};
